<script lang="ts">
	...
	import { getCurrentBreakpoint } from '$lib/utils/twBreakpoint';
	...
	let tabBg = $derived(isOpen ? 'bg-green-500 hover:bg-lime-400 text-green-950' : 'bg-rose-500 hover:bg-red-600');
</script>
...
{#if isOpen}
	...
	>
		<header class=" bg-stone-200 pb-2">
			<X onclick={() => (isOpen = false)} class="  ml-auto size-5 text-stone-500 hover:text-stone-900" />
